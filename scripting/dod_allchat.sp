...
{
	// Whether or not relay chat of dead player's
	if (GetConVarBool(allchat_enable))
	{
		if (IsValidClient(client))
		{
			...
			// For simply 'say' command plugin should relay messages of not alive players
...
public Action:Command_SayTeam(client, const String:command[], argc)
{
	...
	{
		// Check if chat messages sending by valid client
		if (IsValidClient(client))
		{
...
{
	// Check if client in game and not a server
